        ...
        if st.button("🔍 ANALISAR E CORRIGIR"):
            ...
            with st.spinner("IA processando..."):
                ...
                if modo == "IA Resolve (Bancas/Concursos)":
                    # Prompt para a IA resolver a questão sozinha
                    prompt = """
                    ...
                    # Prompt para comparar com gabarito manual
                    prompt = f"""
                ...
                # Envolve chamada externa em try/except para capturar erros de API
                try:
                    log_event("analysis button clicked")

                    if debug_mode:
                        # Resposta simulada para testar fluxo sem chamar API externa
                ...
                # Se temos um resultado, tenta formatar e exibir
                if resultado:
                    try:
                        # Tentar parsear linhas chave: value
                        lines = [l.strip() for l in resultado.splitlines() if l.strip()]
                        ...
                        # Exibição maior e com melhor contraste
                        if 'GABARITO DA IA' in parsed or 'GABARITO' in parsed:
                            gab = parsed.get('GABARITO DA IA', parsed.get('GABARITO'))
                            st.markdown(f"<div style='font-size:26px;font-weight:800;margin-bottom:8px'>Gabarito (IA): <span style='color:#2ecc71'>{gab}</span></div>", unsafe_allow_html=True)

                        # Resumo / corpo com fundo escuro translúcido e texto claro
                        body = parsed.get('RESUMO') if 'RESUMO' in parsed else '\n'.join(lines)
                        st.markdown(
                            f"<div style='background:#0f1724;color:#e6eef8;padding:18px;border-radius:10px;font-size:16px;line-height:1.5;white-space:pre-wrap'>{body}</div>",
                            unsafe_allow_html=True,
                        )
                        ...
                        if status:
                            if 'ACERT' in status.upper():
                                st.markdown(f"<div style='margin-top:12px;font-size:18px;color:#d1fae5;padding:8px;border-radius:6px;background:#04260e'>Status do aluno: {status}</div>", unsafe_allow_html=True)
                            elif 'ERR' in status.upper():
                                st.markdown(f"<div style='margin-top:12px;font-size:18px;color:#ffd6d6;padding:8px;border-radius:6px;background:#3b0b0b'>Status do aluno: {status}</div>", unsafe_allow_html=True)
                            else:
                                st.markdown(f"<div style='margin-top:12px;font-size:18px;padding:8px;border-radius:6px;background:#12303f;color:#cfeff6'>Status do aluno: {status}</div>", unsafe_allow_html=True)

                    except Exception:
                        # Fallback simples e grande
                        st.markdown(f"<div style='font-size:16px;white-space:pre-wrap'>{resultado}</div>", unsafe_allow_html=True)
                else:
